	...
	$("#form_register").submit(function(event)
		{
			...
			$.post("api.php?g=1",$("#form_register").serialize(),function(res)
			{
				...
				$('#alert'+res).fadeIn(1200,"linear").delay(600).fadeOut(1200,"linear");
				...
				if(res==0) {
					...
					$("#usrpwd").animate({height:'+=80px'});
						$("form").animate({marginTop:'-=20px',marginBottom:'-=20px'});
								$('#login_err').fadeIn(1200,"linear").delay(600).fadeOut(1200,"linear");
						$("form").delay(2500).animate({marginTop:'+=20px',marginBottom:'+=20px'});		
	...
	$.get("api.php?g=5",function(res)
		{
			...
			{
				h+="<tr><td>"+js[i].username+"</td><td>"+js[i].email+"</td><td>"+js[i].is_manager+"</td></tr>";
